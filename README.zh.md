...
`--workers` 只控制 CPU 线程数；GPU 模式传入该参数会提示忽略。`--gpu-batch-size` 默认 **262144**，支持 **1–262144**。M4 Pro 持续测试中，262144 约为 **372 万地址/秒**，比相同批次的优化前版本快约 **19.8%**；131072 约为 364 万，低约 2.19%，因此按预先确定的 2% 规则选择 262144。没有启动时自动调优。

批次达到 65536 时，程序用一个 CPU 准备线程生成下一批随机私钥，与 GPU 计算重叠；较小批次保持同步路径。只有两份主机批次、一套 Metal 输入输出缓冲区和一个在途 GPU 批次，不进行 CPU/GPU 混合地址搜索。线程组在 M4 Pro 上继续使用 128；专用平方、快速模加和批量映射实验未证明稳定收益，未启用。

更重视停止响应时可显式使用 `--gpu-batch-size 131072`。本轮停止收尾中位数约为 16ms，262144 约为 56ms；这些是观测值，不是最坏情况保证。完整配对结果、实验淘汰原因及限制见 [性能报告](docs/performance-m4-pro.md#分阶段优化2026-08-28第二轮实施)。
...
这些环境变量只供测试程序使用。基准文件仅记录计数和时间，不保留生成的私钥。诊断计时可用 `VANITY_BENCH_PROFILE=1`；`VANITY_BENCH_PIPELINE=0|1` 可做同步/流水线对照。其余实验开关为 `VANITY_BENCH_BULK=0|1`、`VANITY_BENCH_ADD=0|1`、`VANITY_BENCH_SQUARE=0|1` 和 `VANITY_BENCH_GROUP=auto|32|64|128|256`，普通程序不读取这些开关。正式吞吐比较应关闭诊断计时。
