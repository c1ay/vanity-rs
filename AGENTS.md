...
- `src/backend/` — address derivation: CPU (`cpu.rs`) and Apple Silicon Metal (`metal.rs`, `shader.metal`). Backends fill address slices from valid `SecretKey` batches; they do not match, rank, or write files.
...
- `cargo test` runs unit and CPU CLI tests. Metal tests are `#[ignore]` and require a real device.
