...
`--workers` applies only to CPU; GPU mode warns and ignores it. `--gpu-batch-size` defaults to **262144** (range **1–262144**). On M4 Pro the Metal path uses 16-bit fixed-base windows, two in-flight GPU commands, and increment chains of 32 with affine batched addition (`k·G` once, then `P0 + i·G` for `i < 32` from the fixed-base table, one field inversion per chain). The last clean measurement of the previous kernel was about **29 million addresses/s**; the current kernel measured about **55 million addresses/s** while sharing the GPU with another search process, so the uncontended figure is pending (see the [M4 Pro report](docs/performance-m4-pro.md#链起点上传与仿射批量加2026-09-10)). There is no startup auto-tune.

Hosts only generate, upload, and wipe one **chain start** per 32 addresses; the GPU derives the rest, and the CPU recomputes `start + offset` only for hits and candidates. Batches of 65536 or more overlap the next start batch on one CPU thread with GPU compute. Smaller batches stay synchronous. There are two host key batches and two Metal in/out buffer pairs (at most two in-flight GPU commands). CPU and GPU address search are not mixed. Threadgroups stay at 128 on M4 Pro; dedicated square, fast modular add, bulk-map, and threadgroup Montgomery invert experiments did not show a stable gain and are off.
...
Each worker seeds ChaCha20 from OsRng and rejection-samples valid scalars. GPU batches then walk a short increment chain from each CSPRNG start (`k, k+1, …, k+31` by default) so the kernel can add `i·G` instead of repeating a full scalar multiplication. Starts are rejected when `k < chain length` or `k + chain length - 1 > n - 1`, which are exactly the doubling/infinity exceptions of the incomplete addition formulas (probability about 2^-250 per draw). There is no scan from a low-entropy origin and no production fixed-seed switch.
