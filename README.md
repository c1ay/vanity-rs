...
`--workers` applies only to CPU; GPU mode warns and ignores it. `--gpu-batch-size` defaults to **262144** (range **1–262144**). On M4 Pro, sustained search at 262144 is about **3.72 million addresses/s**, ~19.8% faster than the same batch before the prep pipeline; 131072 is about 2.19% slower, so 262144 was chosen by a pre-set 2% rule. There is no startup auto-tune.

Batches of 65536 or more overlap the next random-key batch on one CPU thread with GPU compute. Smaller batches stay synchronous. There are two host key batches, one Metal in/out buffer pair, and at most one in-flight GPU batch. CPU and GPU address search are not mixed. Threadgroups stay at 128 on M4 Pro; dedicated square, fast modular add, and bulk-map experiments did not show a stable gain and are off.

For faster stop response, use `--gpu-batch-size 131072` (observed stop-tail median ~16 ms vs ~56 ms at 262144; observations, not worst-case bounds). Pairing results and rejected experiments: [M4 Pro report](docs/performance-m4-pro.md#分阶段优化2026-08-28第二轮实施).
