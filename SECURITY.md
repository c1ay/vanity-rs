...
- Keys are produced with `OsRng` → ChaCha20 and rejection sampling of valid secp256k1 scalars. GPU search then uses increment chains (`k, k+1, …`) from those CSPRNG starts so the kernel can add `i·G` instead of repeating a full scalar multiplication. Metal keeps the affine chain points and continues the same chains on later batches; CUDA/Vulkan still expand starts each batch. Only the current chain starts leave the host, and the CPU recomputes `start + offset` for hits and candidates. There is no scan from a low-entropy origin and no production fixed-seed switch.
